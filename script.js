  ...
  let h = `
    <div class="bob">
      ...
      <ul>
        ...
        <button class="btn btn-danger bb2" onclick="window.location.href = 'page404.html';">WATCH</button>
        <button class="btn btn-danger bb2" onclick="window.location.href = 'page404.html';">DOWNLOAD</button>
      </ul>
    </div>
  `;
  aff2.innerHTML = h;
...
function chercher2() {
  ...
  req.onreadystatechange= function(){
    if (req.readyState === XMLHttpRequest.DONE) {
      if (req.status === 200) {
        let movies = JSON.parse(req.response);
        let titre = document.getElementById('titre2').value;
        let h = '';
        for (let i = 0; i < max; i++) {
          if (movies[i].title.toLowerCase().indexOf(titre.toLowerCase()) >= 0) {
            h += `
